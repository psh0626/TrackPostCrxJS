...
export class IcareAPI2 {
    ...
    static async FetchUnreadReplies(csrfToken: string, trialNo = 1, noResponse = 0) {
        ...
        try {
            ...
            if (noResponse >= this.MAX_RETRY_COUNT) {
                await new MSG(CMD.ICARE_UNREAD_REPLIES, "?ICARE").fromContent.toService();
            } else {
                setTimeout(() => {
    ...
    private static async FetchUnreadRequests(csrfToken: string) {
        ...
        if (response.ok && !see_other) {
            ...
            await new MSG(CMD.ICARE_UNREAD_REQUESTS, request_items).fromContent.toService();
        } else {
    ...
    private static async FetchNotifications(csrfToken: string) {
        ...
        if (response.ok && !see_other) {
            ...
            if (isInboundNotiOn) {
                ...
                await new MSG(CMD.ICARE_UNREAD_NOTIF_INBOUND, inbound).fromContent.toService();
            }
            if (this.settings.IcareUnreadNotificationOutbound) {
                ...
                await new MSG(CMD.ICARE_UNREAD_NOTIF_OUTBOUND, outbound).fromContent.toService();
            }
        } else {
    ...
    private static async OnSuccess(response: IcareResponse) {
        ...
        await new MSG(CMD.ICARE_UNREAD_REPLIES, filtered_items).fromContent.toService();
