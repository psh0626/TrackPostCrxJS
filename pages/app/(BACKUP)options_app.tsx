...
export default function OptionsApp() {
  ...
  function onExportClicked() {
    const str_settings = JSON.stringify(settings.current, null, 2);
    const vlink = document.createElement("a");
    const vblob = new Blob([str_settings], { type: "octet/stream" });
    const file_name = "IMICSettings.json";
    const vurl = window.URL.createObjectURL(vblob);
    vlink.setAttribute("href", vurl);
    vlink.setAttribute("download", file_name);
    vlink.click();
    vlink.remove();
  }
      ...
      <Box sx={{ display: "flex", flexGrow: 1, overflow: "auto", height: "88vh" }}>
        ...
        <TabPanel value={tab_value} index={2}>
          <Stack spacing={3} direction="row">
            <Button variant="outlined" color="primary" startIcon={<Upload />}>
              설정 불러오기
            </Button>
            <Button
              variant="outlined"
